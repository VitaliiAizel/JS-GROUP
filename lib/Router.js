const Router = (() => {
     ...
     * @param {HTMLElement} start - The beggining root of search if provided.
     * @returns {ShadowRoot[]} An array of all found shadow roots.
     */
    function findAllShadowRoots(start) {
        const shadowRoots = [];
    ...
    function updateActiveRouterLinks(routerLinkActive) {
        ...
        roots.forEach((root) => {
            ...
            root.querySelectorAll('[routerLinkActive]').forEach((el) => {
            ...
            links.forEach((_, i) => {
                root.querySelectorAll(`[routerLink="${links.slice(0, i + 1).join('/')}"]`).forEach(el => {
                    const togglingClass = el.getAttribute('routerLinkActive');
                    if (togglingClass) {
                        el.classList.add(togglingClass);
                    }
                });
            });
        });
    ...
    async function loadComponent(route) {
        ...
        // Get default data between <router-outlet></router-outlet> without ShadowRoot and remember it
        if (!defaultComponent && findAllShadowRoots(routerOutlet).length == 0) {
            defaultComponent = routerOutlet.innerHTML;
        }

        // Update the router-outlet with the new component
        routerOutlet.innerHTML = `<${component}>${defaultComponent || ''}</${component}>`;
