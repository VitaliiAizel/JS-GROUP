const ComponentWorker = (() => {
    ...
    function proceedNgFor(component, name, data, child) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            tmp = interpolationHelper(component.outerHTML || component, name, data[i]);
    ...
    function interpolate(component, name, data) {
        ...
        const replaced = interpolationHelper(content, name, data);
    ...
    function interpolationHelper(component, groupName, data) {
        return component.replace(/{{(.*?)}}/g, (match, key) => {
            if (!key.includes(".") && key == groupName) {
                return data[key.trim()] || data || '';
            }

            if (key.includes(`${groupName}.`)) {
                return data[key.replace(`${groupName}.`, '').trim()] || data || '';
            }
